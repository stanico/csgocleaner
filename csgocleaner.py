...
def clean():
    ...
    restore_file("\\csgo\\resource\\",# CSGO CLEANER v0.2 11/03/2022
# credits: stanico#1206
# tg: @Stanico
# github.com/stanico/csgocleaner/
...
def restore_file(path, file):
    if os.path.exists(os.path.join(main_path + path + file)):
        ...
        print('remove_byextFILE NOT EXISTS')
        print(file + " " + path)
...
def clean():
    ...
    if b == '1':
        clear()
        intro()
        print('Очистка может занять некоторое времья... зависит от ПК')
    else:
        main()
    main_path = main_path + "\\restorepoint"
...
def main():
    ...
    if not os.path.exists(main_path):
        ...
        if os.path.exists(csgo_path + "\csgo.exe"):
            ...
            clear()
            intro()
            print("Примечание: Перед очисткой желательно посетить вкладку Настройка")
            print('[1] Очистка')
            print('[2] Настройка')
        ...
        if os.path.exists(main_path + "\\restorepoint"):
            if a == "3":
                clear()
                intro()
                print('Восстановление может занять некоторое время... зависит от ПК')
                restore()
                print('Успешно!')
                time.sleep(2)
            if a == "4":
